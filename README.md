...
2. `admin.html`（公開URL＋`/admin.html`）を開く
   - 公開ページからのリンクは無し。URL直打ち or ブックマークでアクセス
3. トークンを貼り付けて **「ログイン」**
   - 有効なトークンで認証が通って初めて編集画面が表示される（＝トークンを持つあなただけが編集可能）
   - トークンはその端末のブラウザにのみ保存され、サイトには送信されません
4. 各日の「担当者名」「メッセージ」を入力 →「保存して公開」
5. 約1分後、公開ページに反映されます

> 代表（6/25〜6/29）の担当者名は未定なので、決まったら管理者ページで名前を入れてください。

### セキュリティの考え方
リポジトリが Public なため `admin.html` というファイル自体は存在しますが、
編集（保存）には **リポジトリへの書き込み権限を持つトークンが必須**です。
そのトークンを発行できるのはオーナーのあなただけなので、他人が管理ページを開いても
ログインできず、何も編集できません。
...
- `COUNTDOWN_TARGET` … カウントダウンの終了時刻（6/30 終日）
- `PERIOD_START` / `PERIOD_END` … 進捗ゲージの集客期間（4/1〜6/30）
